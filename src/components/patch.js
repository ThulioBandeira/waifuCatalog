...
const replaceSomeInformation = (req,res)=>{
    const {
        AnimeManga,
        Name,
        Age,
    ...
    if(!waifu){
        return res.status(404).json({mensagem:"Waifu does not exist in your list"})
    }
    ...
    return res.json({mensagem:"Some informations of Waifu selected has been modified"})
